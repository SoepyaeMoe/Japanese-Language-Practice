...
import n4Kanjis from '../../../data/n4/kanji'
...
const Practice = () => {
    ...
    useEffect(() => {
        if (Array.isArray(n4Kanjis) && n4Kanjis.length) {
            setIdx(getRandomIndex(n4Kanjis.length, null))
        }
    }, [])

    if (!Array.isArray(n4Kanjis) || n4Kanjis.length === 0) {
    ...
    const k = n4Kanjis[idx] || {}

    const handleNext = () => {
        const next = getRandomIndex(n4Kanjis.length, idx)
        setIdx(next)
    }
